...
if args["--train"] != nothing
  ...
  s, ns = args["--kmeans"] ? kmeans_train(files, g = int(args["--gaussians"]), iterations = int(args["--iterations"]), sample_size = 700) :
...
if args["--test"] != nothing
  ...
  threshold, opt_fa, opt_miss, opt_dc, fa, miss, eer, N_speech, N_ns = optimize(files, s, ns)
  @info "optimum threshold = $threshold [eer = $eer, fa rate = $opt_fa, miss rate = $opt_miss, decision cost = $opt_dc]"

  FAs, misses, N, decisions = test(files, s, ns, threshold = threshold)
